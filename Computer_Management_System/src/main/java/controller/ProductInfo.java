...
public class ProductInfo extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			else if (product.matches("Monitor")) {
				rs = statement.executeQuery("select * from monitor where id='"+id+"'");
				if (rs.next() == true) {
					String brand = rs.getString(2);
					String series = rs.getString(3);
					String size = rs.getString(4);
					String resolution = rs.getString(5);
					String refreshRate = rs.getString(6);
					String released = rs.getString(7);
					...
					RequestDispatcher dispatcher = request.getRequestDispatcher("productMonitor.jsp");
					dispatcher.forward(request, response);
				}else {
					RequestDispatcher dispatcher = request.getRequestDispatcher("productError.jsp");
					dispatcher.forward(request, response);
